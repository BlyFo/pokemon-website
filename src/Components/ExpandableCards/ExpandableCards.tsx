        ...
        selectedPokemon.map((pokemon) => (
          <AnimatePresence>
            ...
            onClick={() => setSelectedPokemeon( selectedPokemon.filter((pokemon2) => pokemon2.name !== pokemon.name))}>
              ...
              {pokemon.sprites.other?.['official-artwork']?.front_default && <motion.img className='expandable-card__art' style={{opacity: 1}} layoutId={pokemon.name+"-art"} src={pokemon.sprites.other?.['official-artwork'].front_default} />}
              <motion.div className='expandable-card__type' layoutId={pokemon.name+"-types"}>{ getPokemonTypes(pokemon.types)} </motion.div>
              <motion.div className='expandable-card__description'  initial={{ opacity: 0, height: 0, originY: 0 }} animate={{height: 100, opacity: 1}} transition={{delay: 0.5, duration: 0.8}}>{StringUtils.cleanDescription(pokemon.flavor_text_entries[0].flavor_text)}</motion.div>
          </motion.div>
        </AnimatePresence>
      )))}
    </LayoutGroup>
